 ...
 * - QEMU driver emulation:
 * https://github.com/qemu/qemu/blob/master/hw/timer/cmsdk-apb-timer.c
 *
...
#define CMSDK_TIMER_REGS_GET(_dev) DEVICE_CONFIG_GET(_dev, struct cmsdk_timer)->timer

static inline void timer_handler(const struct device *timer)
{
	struct cmsdk_timer *config = DEVICE_CONFIG_GET(timer, struct cmsdk_timer);
...
static int
set_callback(const struct device *timer, timer_callback_t callback, void *user_data)
{
	int ret				 = -EINVAL;
	struct cmsdk_timer *const config = DEVICE_CONFIG_GET(timer, struct cmsdk_timer);
