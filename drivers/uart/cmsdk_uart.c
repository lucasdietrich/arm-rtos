 ...
 * https://github.com/qemu/qemu/blob/master/hw/char/cmsdk-apb-uart.c
 *
...
#define CMSDK_UART_DEFINE(_name, _uart)                                                  \
	const struct device _name = {                                                    \
		.api	= &api,                                                          \
		.data	= NULL,                                                          \
		.config = &(struct cmsdk_uart_serial_config){.uart = _uart}}
...
static const struct serial_api api = {
	.init = init, .poll_out = poll_out, .poll_in = poll_in};
